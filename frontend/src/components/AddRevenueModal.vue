...
const handleRevenueSubmit = async (revenueData: any) => {
  if (!token.value) return;

  try {
    const payload = {
      ...revenueData,
      amount: parseFloat(revenueData.amount), // ensure it's a number
    };
    ...
    console.log("Saved revenue:", response.data);
    emit("submit", response.data); // optional: return created revenue
  } catch (err: any) {
    console.error("Failed to save revenue:", err?.response?.data || err);
  }
};

const submitForm = () => {
  if (validateForm()) {
    handleRevenueSubmit(formData.value);
    closeModal();
  }
};
</script>
  ...
  <div v-if="isOpen" class="modal-overlay" @click="closeModal">
    <div class="modal-container" @click.stop>
      ...
      <div class="modal-header">
        <h2 class="modal-title">Add New Revenue</h2>
      ...
      <div class="modal-content">
        <form @submit.prevent="submitForm" class="revenue-form">
